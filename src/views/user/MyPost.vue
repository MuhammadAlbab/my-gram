    ...
    <template v-if="itemsByUser.length == 0">
      <v-row align="center" justify="center">
        <v-col class="text-center" cols="12">
          <v-progress-circular :size="50" color="orange" indeterminate>
          </v-progress-circular>
        </v-col>
        <v-col class="text-center" cols="12">
          <h2>
            If the loading takes too long, that means you have not posted
            anything yet!
          </h2>
          <h3>Add one now</h3>
        </v-col>
      ...
      <v-row align="center" justify="center">
        <v-col>
          <v-card class="d-flex flex-wrap justify-space-around" flat tile>
            <v-card
              v-for="(item, index) in itemsByUser"
              :key="item.id"
              class="ma-2"
            >
              <v-img
              ...
              <v-card-text class="pb-0">
                <p class="subtittle-1">
                  {{ item.description | trimLengthDesc }}
                </p>
